...
(function () {
  ...
  const UNPACKED_EXTENSION = '.unpacked'
  ...
  const splitPath = archivePathOrBuffer => {
    ...
    // handle "//some/path.asar/to/archive.asar.unpacked/then/internal.file"...
    if (archivePath.includes(`${ASAR_EXTENSION}${UNPACKED_EXTENSION}${path.sep}`)) {
      return { isAsar: false }
    }
